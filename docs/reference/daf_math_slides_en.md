...
**STEM productions — research first, 2–3 options, one of them AI-integrated.** The production the
students build must be grounded in approved, real-world STEM activity: data collection and display
(NGSS Science and Engineering Practice 4, grades 3–5), authentic data from real research (Data
Nuggets, MSU KBS LTER), standards-aligned project-based learning for the topic (e.g. Playground
Architect PBL, aligned to 4.NBT.B.4), or real schoolyard investigations (Schoolyard SITES, UNH
Extension). Before a topic's productions are written, those sources are researched and listed in
the topic file's `sources` array — `npm run check:stages` fails a deepened topic that ships
without them.

Every lesson's production phase offers the class **2–3 options to build** — a presentation, model,
design, map, song, video, poster, drawing or mini-project, each with its own task and its own STEM
cycle (ask → plan → build → test → share). The class chooses one; the chosen build is named on the
acceptance stamp. **One option always runs AI in the loop:** the student uses the AI to research
the production (find a real example or figure, then verify it) or to challenge it (pose hard
questions, re-check the work independently), and writes or builds every line themselves. This is
grounded in the same research-first rule, with approved AI-integration sources carried in the
topic files' `sources`: the NSTA responsible-use rule — *"Student use of AI should support
learning, not replace it… Work generated by AI is not considered the student's own work"*
(nsta.org, "AI in the Science Classroom"); AI4K12's grades 3–5 band — compare assistants'
answers, and ask *"what might the assistant not know? who is missing from the data?"*; ISTE's
K-12 Classroom AI Projects (students evaluate AI — Knowledge Constructor 3.b, Innovative
Designer 4.a); a peer-reviewed grade-5 chatbot-in-science study where the AI is *a guide that
supplements the teacher, not a replacement* (PMC8211302); and NSTA + STAC's *Sensemaking in the
Age of AI* — AI used purposefully, in classrooms that "preserve curiosity, productive struggle,
and the habits of thinking." The AI option is badged "AI in the loop" on the card and carries the
standing rule: *the AI researches, checks and challenges — it never writes or builds the work.*
Every deck also carries a floor production (two options: the unit artifact as a mini-project,
plus an AI-researched poster option), so the production phase is never empty.
