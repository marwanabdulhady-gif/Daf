/* ===========================================================================
   Grade 4 · Topic 9 · LESSON 9-4 · Model Subtraction of Fractions
   Standard 4.NF.B.3   I can ... subtract fractions with the same denominator by
   taking pieces away on a model.
   =========================================================================== */

const M = {
  a: om(mfrac(7, 8)),
  b: om(mfrac(3, 8)),
  answer: om(mfrac(7, 8), mt("-"), mfrac(3, 8), mt("="), mfrac(4, 8)),
  unitCount: om(mnor("7 eighths take away 3 eighths is 4 eighths")),
...
const makeBar94 = (step) => (ctx, W, H, frame) => {
  D.rr(ctx, 0, 0, W, H, 14);
  ctx.fillStyle = "#0B1F24"; ctx.fill();
  const x = 66, w = W - 132;
  const shown = step >= 1 ? 4 : 7;
...
const makeLine94 = (on) => (ctx, W, H, frame) => {
  D.rr(ctx, 0, 0, W, H, 14);
  ctx.fillStyle = "#0B1F24"; ctx.fill();
...
const drawStory94 = (ctx, W, H, frame) => {
  const CYCLE = 620, f = frame % CYCLE;
  const p1 = D.at(f, 0, 130), p2 = D.at(f, 130, 340), p3 = D.at(f, 350, 470);
  D.rr(ctx, 0, 0, W, H, 14);
  ctx.fillStyle = "#0B1F24"; ctx.fill();
  D.txt(ctx, "a jug of laban", W / 2, 22, { size: 12.5, col: "#C9A227", font: "marker", alpha: p1 });
  D.fracBar(ctx, { x: 66, y: 58, w: W - 132, h: 50, den: 8, num: 7, prog: p2, col: "#FA7E19" });
  if (p3 > 0) {
    D.txt(ctx, "the jug was 7 eighths full, 3 eighths were poured out", W / 2, 150, { size: 13.5, col: "#EAF4F2", font: "marker", alpha: p3 });
    D.txt(ctx, "how much is left in the jug?", W / 2, H - 16, { size: 14.5, col: "#C9A227", font: "marker", alpha: p3 });
  }
};

const drawBoard94 = (ctx, W, H, frame) => {
  ...
  if (p2 > 0) {
    ...
    ctx.restore();
    D.txt(ctx, "the 3 removed", x + (4 + 3 / 2) * pw, 96,
...
const LESSON = {
  code: "9-4",
  storageKey: "daf-g4-t9-l4",
  title: "Model Subtraction of Fractions",
  unit: "GRADE 4 · TOPIC 9 · LESSON 9-4 · 4.NF.B.3",
  math: M,
  ixl: ["PXZ", "QAS", "MJX", "P99"],
  ...
  metas: [
    ...
    { phase: "launch", title: "Seven eighths, then <em>three poured out</em>", lead: "The jug was 7 eighths full. 3 eighths were poured out. Estimate what is left.",
      ...
      rail: { launch: "How much is left? Do not calculate — look at the bar.",
        ...
        misconception: "Answering 4/0 or 4/16 by touching the denominators." } },

    { phase: "monitor", title: "Take the <em>pieces</em> away", lead: "Shade the starting amount, then remove the pieces.",
      ...
      rail: { launch: "Predict the answer before you step.",
        ...
        connect: "Why does the bottom number stay 8?", misconception: "Changing the denominator when subtracting." } },
    ...
    { phase: "connect", title: "Two ways to <em>say it</em>", lead: "Nour counted the eighths that were left. Adel subtracted the top numbers. Both got four eighths.",
    ...
    { phase: "synth", title: "On the <em>board</em>", lead: "Seven eighths. Take three eighths. Four eighths. The eighth never changed.",
      goal: "The moment the lesson is taught — not displayed.",
      pull: "Say it in one sentence.",
      rail: { launch: "Draw it with them, do not present it to them.",
        monitor: ["Predicting the result bar", "Naming the unit", "Restating it in their own words"],
        connect: "Who can say the rule in one sentence?",
        misconception: "Writing 4/0 because both numbers were subtracted." } },
    ...
    { phase: "swyk", title: "<em>Show</em> what you know",
      ...
      rail: { launch: "Two minutes. Watch the denominator.",
        ...
        misconception: "Answering 3/0 by subtracting the denominators." } },

    { phase: "connect", title: "What you <em>collected</em> today",
      lead: "Points are for thinking, not for speed.",
      goal: "Close on one action a student can actually do tonight.",
      pull: "Tomorrow: doing it without the picture.",
  ...
  Visual: function ({ i, award, game }) {
    ...
    switch (i) {
      case 0:
        return <NoticeWonder draw={makeBar94(2)} height={256} award={award}
          notices={["The bar is in eighths", "Most of it is shaded", "A dashed box holds three", "All the pieces match"]} wonders={["How much will be left?", "Does the bottom number change?", "Can I just subtract the tops?"]} />;

      case 1:
        return <LaunchEstimate draw={drawStory94} height={256} award={award}
      ...
      case 2:
        return <ExploreChips draw={makeBar94(step)} height={256}
      ...
      case 3:
        return <ExploreChips draw={makeLine94(on)} height={256}
      ...
      case 4:
        return <CardSort award={award} columns={2}
          items={[{ id: "s1", text: "7/8 − 3/8 = 4/8", target: "yes" }, { id: "s2", text: "7/8 − 3/8 = 4/0", target: "no" }, { id: "s3", text: "5/6 − 2/6 = 3/6", target: "yes" }, { id: "s4", text: "5/6 − 2/6 = 3/0", target: "no" }]}
          targets={[
      ...
      case 5:
        return <CompareConnect award={award}
          left={{ name: "Nour's way — count what is left", omml: M.unitCount, h: 92, quote: "After three go, I count the rest: four eighths." }}
          right={{ name: "Adel's way — subtract the tops", omml: M.answer, h: 92, quote: "Seven take three is four, still eighths." }}
          same={["Both get 4/8", "Both keep the denominator", "Both treat an eighth as one thing"]} diff={["Nour counts, Adel subtracts", "Adel is faster", "Nour can see why the bottom stays"]} />;

      case 6:
        return <BoardScreen draw={drawBoard94} height={430} />;
      ...
      case 8:
        return <ShowWhatYouKnow award={award}
          prompt="What is five sixths minus two sixths?" omml={M.swyk} options={[{ v: "a", text: "3/0" }, { v: "b", text: "3/6" }, { v: "c", text: "7/6" }, { v: "d", text: "3/12" }]} right="b"
          support={{ yes: "Yes — 5 sixths take 2 sixths is 3 sixths.", notYet: "Not yet — the pieces do not change size.",
            draw: drawSupport94, h: 92, hint: "You are counting sixths, so what is left is in sixths." }} />;

      case 9:
        return <Closing game={game} omml={M.rule} action="Pour out part of a drink at home and describe what is left as a fraction." />;
