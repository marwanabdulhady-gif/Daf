...
@media(max-width:900px){.hero-in{grid-template-columns:1fr}.lantern{display:none}.grid,.lanes{grid-template-columns:1fr}.lane{border-right:0;border-bottom:1px solid var(--line)}.loop{grid-template-columns:repeat(2,1fr)}.unit-grid{grid-template-columns:1fr}.lesson{grid-template-columns:56px 1fr}.lesson>div:nth-child(3),.lesson>div:nth-child(4){grid-column:2}.roadmap{grid-template-columns:1fr 1fr}}
@media(max-width:560px){.hero{padding-top:44px}.wrap{padding-inline:15px}.grid,.guardrails,.roadmap{grid-template-columns:1fr}.loop{grid-template-columns:1fr}.meta{grid-template-columns:1fr}.lesson{display:block}.lesson>div{margin-bottom:8px}.toolbar{top:52px;flex-wrap:wrap}.search{width:100%}.hero h1{font-size:42px}}
...
<section class="section" id="loop">
  <div class="eyebrow">Repeatable lesson grammar</div><h2>The story lives inside the existing ten-screen lesson arc.</h2>
  <p class="intro">No extra “story slides” are needed. The narrative gives the mathematics a need, then gets out of the way.</p>
  <div class="loop" id="loopGrid"></div>
  <div class="callout"><p><b>Time budget:</b> 20–30 seconds for “previously,” up to 60 seconds for the new hook, then mathematics. The closing handoff takes about 30–45 seconds. If a class can advance the plot without doing the mathematics, redesign it.</p></div>
</section>
...
<section class="section" id="all-lessons">
  ...
  <p class="intro">This is the content-authoring map, not a claim that all story text is already embedded in the decks. Search by lesson, technique, mission or standard.</p>
...
<footer class="footer"><b>The Lantern of Numeria</b> · planning proposal generated from <code>story/story-map.json</code> · 17 units · 114 lesson beats · Seerah content requires qualified school review before student release.</footer>
...
DATA.lessonStoryLoop.forEach((x,i)=>{const el=document.createElement("article");el.className="loop-step";el.innerHTML='<b>'+esc(x.screen)+'</b><p>'+esc(x.move)+'</p>';loop.appendChild(el)});
...
function renderLessons(){const q=search.value.trim().toLowerCase(),f=filter.value;list.innerHTML="";let shown=0;DATA.units.forEach(u=>{if(f&&String(u.topic)!==f)return;const matches=u.lessons.filter(l=>!q||Object.values(l).join(" ").toLowerCase().includes(q)||u.chapter.toLowerCase().includes(q));if(!matches.length)return;shown+=matches.length;const d=document.createElement("details");d.className="lesson-unit";d.open=!!q||!!f;d.innerHTML='<summary>Topic '+u.topic+' · '+esc(u.chapter)+'<span>'+matches.length+' lesson'+(matches.length===1?'':'s')+'</span></summary><div class="lesson-table"></div>';const table=d.querySelector(".lesson-table");matches.forEach(l=>{const row=document.createElement("article");row.className="lesson";row.innerHTML='<div><span class="code">'+esc(l.code)+'</span><small>'+esc(l.standard)+'</small></div><div><b>'+esc(l.storyBeat)+'</b><small>'+esc(l.curriculumTitle)+'</small></div><div><span class="tech">'+esc(l.technique)+'</span><small>'+esc(l.storyMove)+'</small></div><div><b>Student mission</b><small>'+esc(l.studentMission)+'</small><small><b>Handoff · </b>'+esc(l.handoff)+'</small></div>';table.appendChild(row)});list.appendChild(d)});count.textContent=shown+' of 114 lessons'}
