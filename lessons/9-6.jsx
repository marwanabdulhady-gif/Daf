/* ===========================================================================
   Grade 4 · Topic 9 · LESSON 9-6 · Add and Subtract Fractions with Like Denominators
   Standard 4.NF.B.3   I can ... add and subtract fractions with the same
   denominator, and choose which one the problem needs.
   =========================================================================== */
...
const makeBar96 = (step) => (ctx, W, H, frame) => {
  D.rr(ctx, 0, 0, W, H, 14);
...
const makeLine96 = (on) => (ctx, W, H, frame) => {
  D.rr(ctx, 0, 0, W, H, 14);
  ctx.fillStyle = "#0B1F24"; ctx.fill();
...
const drawStory96 = (ctx, W, H, frame) => {
  const CYCLE = 620, f = frame % CYCLE;
  const p1 = D.at(f, 0, 130), p2 = D.at(f, 130, 340), p3 = D.at(f, 350, 470);
  D.rr(ctx, 0, 0, W, H, 14);
  ctx.fillStyle = "#0B1F24"; ctx.fill();
  D.txt(ctx, "the water bottle, filled then used", W / 2, 22, { size: 12.5, col: "#C9A227", font: "marker", alpha: p1 });
  D.fracBar(ctx, { x: 66, y: 58, w: W - 132, h: 50, den: 8, num: 3, prog: p2, col: "#FA7E19" });
  if (p3 > 0) {
    D.txt(ctx, "4 tenths in, then 5 more in, then some used", W / 2, 150, { size: 13.5, col: "#EAF4F2", font: "marker", alpha: p3 });
    D.txt(ctx, "does this need adding or subtracting?", W / 2, H - 16, { size: 14.5, col: "#C9A227", font: "marker", alpha: p3 });
  }
};

const drawBoard96 = (ctx, W, H, frame) => {
...
const LESSON = {
  code: "9-6",
  storageKey: "daf-g4-t9-l6",
  title: "Add and Subtract Fractions with Like Denominators",
  unit: "GRADE 4 · TOPIC 9 · LESSON 9-6 · 4.NF.B.3a",
  math: M,
  ixl: ["GAK", "FXD"],
  ...
  metas: [
    ...
    { phase: "launch", title: "Add, or <em>subtract</em>?", lead: "Some problems join amounts. Some take them away. The words decide. Estimate the total first.",
    ...
    { phase: "monitor", title: "Choose the <em>operation</em>", lead: "Read the story. Join means add. Remove means subtract. The denominator never changes either way.",
      goal: "One rule covers both operations: the pieces never change size.", pull: "The same move looks different on a line.",
    ...
    { phase: "swyk", title: "<em>Show</em> what you know",
      ...
      rail: { launch: "Two minutes. Watch the denominator.",
        ...
        misconception: "Adding instead of subtracting because the numbers look like an addition." } },

    { phase: "connect", title: "What you <em>collected</em> today",
      lead: "Points are for thinking, not for speed.",
      goal: "Close on one action a student can actually do tonight.",
      pull: "Tomorrow: fractions bigger than one whole.",
  ...
  Visual: function ({ i, award, game }) {
    ...
    switch (i) {
      case 0:
        return <NoticeWonder draw={makeBar96(2)} height={256} award={award}
      ...
      case 1:
        return <LaunchEstimate draw={drawStory96} height={256} award={award}
      ...
      case 2:
        return <ExploreChips draw={makeBar96(step)} height={256}
      ...
      case 3:
        return <ExploreChips draw={makeLine96(on)} height={256}
      ...
      case 6:
        return <BoardScreen draw={drawBoard96} height={430} />;
      ...
      case 8:
        return <ShowWhatYouKnow award={award}
          prompt="A jug had 7/8 and 3/8 was poured out. How much is left?" omml={M.swyk} options={[{ v: "a", text: "10/8" }, { v: "b", text: "4/8" }, { v: "c", text: "4/0" }, { v: "d", text: "4/16" }]} right="b"
          support={{ yes: "Yes — 2 sixths and 3 sixths is 5 sixths.", notYet: "Not yet — the pieces do not change size.",
            draw: drawSupport96, h: 92, hint: "Poured out means the amount goes down, so subtract." }} />;

      case 9:
        return <Closing game={game} omml={M.rule} action="Find one adding and one subtracting fraction situation at home tonight." />;
